...
class Hunter{
  ...
  Hunter(float x, float y){
   ...
   void run(ArrayList<Hunter> h, ArrayList<PreyBoids> pb){
    update();
    borders();
    render();
    checkSpeed();
  ...
  void applyForces(PVector s, PVector a, PVector c){
    accel.add(s);
    accel.add(a);
    accel.add(c);
  }
    
  void preyFlock(ArrayList<PreyBoids> pb){
  ...
  PVector separate(ArrayList<PreyBoids> pb){
    float distance = 5.0f;
  ...
  PVector align(ArrayList<PreyBoids> pb){
    float neighbor = 75;
  ...
  PVector cohesion(ArrayList<PreyBoids> pb){
    float neighbor = 75;
    PVector sum = new PVector (0, 0);
    int count = 0;
...
class PreyBoids{
  ...
  void run(ArrayList<Hunter> hunter, ArrayList<PreyBoids> pb){
    flock(hunter, pb);
    checkLife(pb);
    update(pb);
    borders();
  ...
  //remove if eaten
  void checkIfEaten(Hunter h, ArrayList<PreyBoids> pb){
    float dist = 5f;
  ...
  //remove if old
  void checkLife(ArrayList<PreyBoids> pb){
  ...
  void checkForBabies(ArrayList<PreyBoids> pb){
    float distance = 25f;
    for(int i = 0; i < pb.size(); i++){
      PreyBoids neighbor = pb.get(i);
  ...
  void flock(ArrayList<Hunter> hunter, ArrayList<PreyBoids> pb){
    PVector separate = separate(pb);
  ...
  void update(ArrayList<PreyBoids> pb){
    vel.add(accel);
    vel.limit(dna[2]);
    loc.add(vel);
    accel.mult(0);
  ...
  PVector align(ArrayList<Hunter> hunter, ArrayList<PreyBoids> pb){
    float neighbor = 50;
